...
impl BoardContract for Board {
    ...
    fn is_check(&self, color: Color) -> bool {
        ...
        self.is_square_threatened(king_position, color)
    }
...
impl Board {
    ...
    fn is_square_threatened(&self, position: &(u8, u8), color: Color) -> bool {
        ...
        for piece in self.get_pieces_iter(color.opposite()) {
            if piece.get_moves(&rival_team, &team).contains(position) {
                return true;
            }
        }
        false
...
mod tests {
    ...
    impl Board {
        pub fn do_move(&mut self, position: &str, target: &str) {
            ...
            if self.is_castling_move(&position, &target) {
                self.castle(&position, target)
            } else {
    ...
    #[test]
    fn castling_on_queen_side() {
        let mut board = Board::new();
        let moves = read_to_string("games/castling.txt").unwrap();
        board.do_moves(moves.split_whitespace().collect());
        assert!(board.get_square_color(&"e1".as_u8().unwrap()).is_none());
        assert_eq!(board.get_piece_type(&"d1".as_u8().unwrap()), "Rook");
        assert_eq!(board.get_king_position(Color::White), &"c1".as_u8().unwrap());
        assert_eq!(board.get_square_color(&"a1".as_u8().unwrap()), None);
    }
}
